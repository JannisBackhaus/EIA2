...
(function (Aufgabe09) {
    ...
    function createSnowflakes(amount) {
        for (let i = 0; i < amount; i++) {
            ...
            let x = Math.random() * flake.canvas.width;
            let y = Math.random() * flake.canvas.height;
    ...
    function createTrees(type, amount) {
        if (type == true) {
            for (let i = 0; i < amount; i++) {
                let tree = canvas.getContext("2d");
                let x = (Math.random() * (tree.canvas.width / 3)) + 2 * (tree.canvas.width / 3);
                let y = (Math.random() * (tree.canvas.height / 2)) + (tree.canvas.height / 2);
            ...
            for (let i = 0; i < amount; i++) {
                let tree = canvas.getContext("2d");
                let x = (Math.random() * (tree.canvas.width / 3) + tree.canvas.width / 5);
                let y = (Math.random() * (tree.canvas.height / 5)) + 4 * (tree.canvas.height / 5);
                let size = (Math.random() * 5) + 20;
                tree.beginPath();
                tree.moveTo(x, y);
                tree.lineTo(x - 100, y);
